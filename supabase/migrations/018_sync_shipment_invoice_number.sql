-- Replace create_shipment_with_invoice to ensure shipment_no and invoice_reference
-- are EXACTLY the same as the generated invoice_number.
...
begin
  ...
  -- Ensure shipment_no and invoice_reference match the generated invoice number exactly
  update public.shipments
  set shipment_no = v_invoice_number,
      invoice_reference = v_invoice_number
  where id = v_shipment_id;

  -- Reload shipment row to reflect new numbers in snapshot
  select * into v_shipment
  from public.shipments
  where id = v_shipment_id;
  ...
  insert into public.invoices (
    ...
    current_date,
    current_date + coalesce(v_settings.invoice_due_days, 30),
    'draft',
    v_client_snapshot,
    v_shipment_snapshot,
    ...
    coalesce(current_setting('request.jwt.claim.sub', true)::uuid, auth.uid()),
    coalesce(current_setting('request.jwt.claim.sub', true)::uuid, auth.uid())
  ...
  insert into public.invoice_items (
    ...
    v_invoice_id,
    'transport',
    'TRIP FROM: ' || coalesce(v_shipment.loading_point, '') || ' TO ' || coalesce(v_shipment.destination, ''),
    1,
    v_shipment.company_rate,
    v_shipment.company_rate,
    jsonb_build_object(
